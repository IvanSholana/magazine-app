...
<body class="font-[Poppins] pb-[83px]">
	...
	<nav id="Category" class="max-w-[1130px] mx-auto flex justify-center items-center gap-4 mt-[30px]">
		...
		<a href="{{route('front.category',$item->name)}}"
	...
	<section id="author" class="max-w-[1130px] mx-auto flex items-center flex-col gap-[30px] mt-[70px]">
		<div id="title" class="flex items-center gap-[30px]">
			...
			<div class="flex gap-3 items-center">
				<div class="w-[60px] h-[60px] flex shrink-0 rounded-full overflow-hidden">
					<img src="{{Storage::url($current_author->avatar)}}" alt="profile photo" />
				</div>
				<div class="flex flex-col">
					<p class="text-lg leading-[27px] font-semibold">{{$current_author->name}}</p>
					<span class="text-[#A3A6AE]">{{$current_author->occupation}}</span>
				</div>
			</div>
		</div>
		<div id="content-cards" class="grid grid-cols-3 gap-[30px]">
			...
			@empty
				<p>Tidak ada postingan terkini...</p>	
			@endforelse
		</div>
	</section>
