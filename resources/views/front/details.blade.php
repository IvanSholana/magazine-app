	...
	<section id="Article-container" class="max-w-[1130px] mx-auto flex gap-20 mt-[50px]">
		...
		<div class="side-bar flex flex-col w-[300px] shrink-0 gap-10">
			<div class="ads flex flex-col gap-3 w-full">
				<a href="">
					<img src="{{Storage::url($banneradssquare->thumbnail)}}" class="object-contain w-full h-full" alt="ads" />
				</a>
			...
			<div id="More-from-author" class="flex flex-col gap-4">
				...
				@forelse ($otherArticle as $other)
					<a href="{{route('front.details',$other->slug)}}" class="card-from-author">
						<div class="rounded-[20px] ring-1 ring-[#EEF0F7] p-[14px] flex gap-4 hover:ring-2 hover:ring-[#FF6B18] transition-all duration-300">
							<div class="w-[70px] h-[70px] flex shrink-0 overflow-hidden rounded-2xl">
								<img src="{{ Storage::url($other->thumbnail) }}" class="object-cover w-full h-full" alt="thumbnail">
							</div>
							<div class="flex flex-col gap-[6px]">
								<p class="line-clamp-2 font-bold">{{ $other->name }}</p>
								<p class="text-xs leading-[18px] text-[#A3A6AE]">{{ $other->created_at->format('d M, Y') }} • {{ $other->category->name }}</p>
							</div>
						</div>
					</a>
				@empty
					<p>Tidak ada postingan lain...</p>
				@endforelse
			</div>
			<div class="ads flex flex-col gap-3 w-full">
				<a href="">
